          ...
          <div className='grid relative normal grid-cols-1 md:grid-cols-2 p-4 gap-8 md:p-8' >
            ...
            {data.filter((data) => data.value === userData["paymentCrypto"]).map((data) => {
              ...
              return(
                ...
                onClick={() =>notify(data.address)}
                className='text-green-400 cursor-pointer break-all' >{data.address}</span> and you'll receive {recieveAmount} Jkwon. Please make sure the amount is correct to avoid any delays. Once you have initiated your payment please click submit and our system will notify you when we have received and cleared your funds</p>
               <input type="text"
